...
namespace {
      ...
      io_service_pool * pool)
  {
    ...
    (void)session->timer->expires_from_now(boost::posix_time::milliseconds(session->timeout));
...
int main(int argc, char ** argv)
{
  try
  {
    ...
    boost::asio::ip::tcp::endpoint endpoint(
        boost::asio::ip::address::from_string("127.0.0.1"), static_cast<unsigned short>(port));

    io_service_pool pool(static_cast<size_t>(threadpool_size));

    simulate_client(clients, endpoint, &pool);

    boost::thread_group tg;
    (void)tg.create_thread(boost::bind(&io_service_pool::run, &pool, true));
    tg.join_all();
  }
